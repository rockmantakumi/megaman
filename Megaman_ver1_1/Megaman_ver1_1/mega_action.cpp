...
void draw_megaman(Mega Current) {
	if (Megaman.reverse == 0) {//右向き
		if (Megaman.jampFlag == 1) {//ジャンプ可能
			if (Megaman.attackFlag == 1) {
				//攻撃していてジャンプを押したときに球が消えないようにしている
				mega_jamp(Rx, Ry, Megaman.colorCondition);
				megaman_attack();
				Ry_temp = Ry;

				if (Ry > TATECELL - 60) {//高さは地面から６０まで上がる
					Ry += (Ry - Ry_prev) + 1;
				}
				else if (Ry <= TATECELL - 60) {//高さを60までに制限
					Ry = TATECELL - 59;
				}
				Ry_prev = Ry_temp;
				if (Ry >= TATECELL) {
		...
		else if (Megaman.moveFlag == 2) {//右に移動
		...
		else if (Megaman.slidFlag == 1) {//スライディングする
			slid_megaman();
	...
	if (Megaman.attackFlag == 1 && Megaman.jampFlag == 1) {//攻撃していてジャンプを押したときに球が消えないようにしている
		megaman_attack();
	}
...
void megaman_Buster1() {//攻撃メソッド
	...
	if (Bx < 3 || Bx > YOKOCELL - 24) {//壁まで到着したら球は消える
		Bx = Rx;
		megaattackNum++;
	}
	else if (Bx > YOKOCELL - 24) {
		Bx = Rx + 25;
		megaattackNum++;
	}
	hit_limit(0, 0, 1);
