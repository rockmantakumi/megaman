...
void Enemy_efect() {
	...
	int Center_x = Ex + Current.width / 2;	//円の中心のｘ座標
	int Center_y = Ey - Current.height / 2;	//円の中心のｙ座標
	int cnt = 0;

	while (cnt < 10) {

		//直径が敵の高さ分の円を描画し、点滅させる
		for (int y = TATECELL - Current.height; y < TATECELL; y++) {
			for (int x = Center_x - Current.height / 2; x < Center_x + Current.height / 2; x++) {
				if (((int)pow(x - Center_x, 2)) + ((int)pow(y - Center_y, 2)) < (int)pow(Current.height / 2, 2)) {
					if (cnt % 2 == 0) {
